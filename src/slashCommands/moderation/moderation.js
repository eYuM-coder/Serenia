...
const { checkModPermissions } = require("../../utils/permissionsChecker.js");
    ...
    if (subcommandGroup === "channel") {
      ...
      if (subcommand === "lock") {
        ...
        try {
          ...
          if (channel.type != "GUILD_TEXT" || !channel.viewable) {
            let channelerror = new EmbedBuilder()
              .setAuthor({
      ...
      if (subcommand === "ban") {
        ...
        try {
          ...
          if (response) {
            let timeoutsuccess = new EmbedBuilder()
              .setColor("Green")
              .setDescription(
                `***${
        ...
        try {
          ...
          if (!warnDoc || !warnDoc.warnings.length) {
            let nowarnerror = new EmbedBuilder()
              .setAuthor({
                name: `${interaction.user.tag}`,
                iconURL: interaction.member.displayAvatarURL({ dynamic: true }),
              })
          ...
          if (!warnID) {
            let warnIDinvalid = new EmbedBuilder()
              .setAuthor({
                name: `${interaction.user.tag}`,
                iconURL: interaction.member.displayAvatarURL({ dynamic: true }),
              })
          ...
          if (!warnDoc.warningID.includes(warnID)) {
            let warnremoveerror = new EmbedBuilder()
              .setAuthor({
                name: `${interaction.user.tag}`,
                iconURL: interaction.member.displayAvatarURL({ dynamic: true }),
              })
        ...
        try {
          ...
          if (mentionedMember) {
            interaction
              .reply({
                embeds: [
                  new EmbedBuilder().setColor(client.color.green)
