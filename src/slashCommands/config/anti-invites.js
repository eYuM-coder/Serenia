      ...
      await Guild.findOne(
        ...
        async (err, guild) => {
          ...
          let embed = new MessageEmbed()
            ...
            .setDescription(
              `${interaction.client.emoji.success} | ${language.antiinvites3}`
            );
          return interaction
            .reply({ embeds: [embed] })
            .then(async () => {
    ...
    if (toggle.toLowerCase() === "enable") {
      if (guildDB.antiInvites === true) {
        let embed = new MessageEmbed()
          .setColor(interaction.client.color.red)
          .setDescription(
            `${interaction.client.emoji.fail} | ${language.moduleEnabled}`
          );
        return interaction
          .reply({ embeds: [embed] })
          .then(async () => {
      ...
      await Guild.findOne(
        ...
        async (err, guild) => {
          ...
          let embed = new MessageEmbed()
            .setColor(interaction.client.color.green)
            .setDescription(
              `${interaction.client.emoji.success} | ${language.antiinvites4}`
            );
          return interaction
            .reply({ embeds: [embed] })
            .then(async () => {
