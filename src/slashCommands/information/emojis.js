...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const embed = new MessageEmbed()
      .setAuthor({
        name: `${interaction.guild.name}'s Emojis`,
    ...
    if (emojis.length === 0)
      interaction.reply({
        embeds: [embed.setDescription(`${language.emoji2}`)],
      });
    else if (emojis.length <= interval) {
      const range = emojis.length == 1 ? "[1]" : `[1 - ${emojis.length}]`;
      interaction.reply({
        content: `This guild has ${emojis.length} emojis.`,
        ephemeral: true,
        embeds: [
          embed
            .setAuthor({
            ...
            .setTitle(`${language.emoji1} ${range}`)
            .setDescription(emojis.join("\n")),
        ],
      });
    } else {
      interaction
        .reply({ content: `This guild has ${emojis.length} emojis.` })
        .then(async () => {
          setTimeout(() => {
