...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let embedValid = new EmbedBuilder()
      .setAuthor({
    ...
    if (members.length === 0)
      ...
      interaction.reply({
        embeds: [
          embed
            .setTitle(`${capitalize(role.name)} ${language.list} ${range}`)
      ...
      embed
        .setTitle(`${capitalize(role.name)} ${language.list}`)
      ...
      interaction.reply({
        content: `The message has been sent below.`,
        ephemeral: true,
      });
