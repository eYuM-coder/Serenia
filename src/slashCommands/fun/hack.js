    ...
    try {
      ...
      if (!user) {
        return interaction.reply({
          content: `${language.hack1}`,
          ephemeral: true,
        });
      } else {
        if (user.bot) {
          return interaction.reply({
            content: `${language.hackbot}`,
            ephemeral: true,
          });
        }
      }
      ...
      interaction
        ...
        .then(async (msg) => {
          setTimeout(async function () {
            await interaction
              .editReply({ content: `[▘] ${language.hack13}` })
              .catch(() => {});
          }, 1500);
          setTimeout(async function () {
            await interaction
              .editReply({
                content: `[▝] Email: \`${darkemail({
                  domain: "gmail.com",
          ...
          setTimeout(async function () {
            await interaction
              .editReply({ content: `[▘] ${language.hack14}` })
              .catch(() => {});
          }, 6000);
          setTimeout(async function () {
            await interaction
          ...
          setTimeout(async function () {
            await interaction
              .editReply({ content: `[▗] Finding IP address...` })
              .catch(() => {});
          }, 9000);
          setTimeout(async function () {
            await interaction
              .editReply({
                content: `[▖] IP address: \`127.0.0.1:${darkrandom.int(
                  100,
                  9999
                )}\``,
              })
          ...
          setTimeout(async function () {
            await interaction
              .editReply({ content: `[▘] ${language.hack15}` })
              .catch(() => {});
          }, 12000);
          setTimeout(async function () {
            await interaction
              .editReply({ content: `[▝] ${language.hack16}` })
              .catch(() => {});
          }, 13500);
          setTimeout(async function () {
            await interaction
              .editReply({
                content: `${language.hack17} ${member.user.username}`,
              })
              .catch(() => {});
          }, 15000);
          setTimeout(async function () {
