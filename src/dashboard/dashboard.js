...
module.exports = async (client) => {
  ...
  app.get("/api/thresholds", async (req, res) => {
    const file = path.join(__dirname, "api", "thresholds", "thresholds.json");

    res.setHeader("Access-Control-Allow-Origin", "*");

    if (!fs.existsSync(file)) {
      return res.status(404).json({ error: "Thresholds not set." });
    }
    ...
    if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
      ...
      if (ticketSettings.ticketToggle == "true") {
        if (
          ...
          if (storedSettings.isPremium == "false") {
            ticketSettings.ticketFooter = "Powered by serenia.eyum.org";
          } else {
          ...
          let reactionFooter = "Powered by serenia.eyum.org";

          let footer = "Powered by serenia.eyum.org";
          ...
          if (storedSettings.isPremium == "false") {
            ticketEmbed.setFooter({ text: `Powered by serenia.eyum.org` });
          } else {
            if (checkFooter) {
