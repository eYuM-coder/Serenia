...
module.exports = async (client) => {
  ...
  app.get("/logout", async function (req, res) {
    ...
    req.session.destroy(() => {
      req.logout((err) => {
        if (err) console.error("An error occured when trying to logout:", err);
      });
      res.redirect("/");
    });
  });
  ...
  app.get("/apply", checkAuth, (req, res) => {
    renderTemplate(res, req, "appeal.ejs", {
      perms: Discord.PermissionFlagsBits,
      botName: config.botName,
    });
  });
  ...
  app.post("/redeem/:guildID", checkAuth, async (req, res) => {
    ...
    const embedPremium = new Discord.EmbedBuilder()
      .setDescription(
...
In the mean time, please explain your issue below`;
    ...
    if (Object.prototype.hasOwnProperty.call(data, "sendEmbed")) {
      ...
      if (ticketSettings.ticketToggle == "true") {
        if (
          ...
          if (embedColor == "#000000")
            embedColor = guild.members.me.displayHexColor;
          ...
          if (storedSettings.isPremium == "false") {
            ticketSettings.ticketFooter = "Powered by serenia.eyum.dev";
          } else {
          ...
          let reactionFooter = "Powered by serenia.eyum.dev";

          let footer = "Powered by serenia.eyum.dev";
          ...
          if (storedSettings.isPremium == "false") {
            ticketEmbed.setFooter({ text: `Powered by serenia.eyum.dev` });
          } else {
            if (checkFooter) {
  ...
  app.post("/dblwebhook", webhook.middleware(), async (req) => {
    ...
    const msg = new Discord.EmbedBuilder()
      ...
      .setDescription(
        `Thank you **${apiUser.username}#${apiUser.discriminator}** (${apiUser.id}) for voting **Serenia**!`,
      );
    Hook.sendCustom(msg);
