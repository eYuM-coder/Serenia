...
$(document).ready(() => {
  ...
  function updateEmbed(embed) {
    ...
    if (embed.url) {
      ...
      $(".embed-inner").append(
        `<div class="embed-title">${embed.title.match(/.{1,46}/g).join("<br/>")}</div>`,
      );
    }
    ...
    if (embed.author.name) {
      $(".embed-title").before(
        `<div class="embed-author"><a class="embed-author-name" href="${embed.author.url}">${embed.author.name.match(/.{1,46}/g).join("<br/>")}</a></div>`,
      );
