...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    if (!message.member.PermissionsBitField.has("MANAGE_MESSAGES"))
    ...
    if (!channel.permissionsBitFieldFor(message.guild.members.me).has(["SEND_MESSAGES"]))
      return message.channel.sendCustom(`${language.sendmessages}`);

    if (!channel.permissionsBitFieldFor(message.member).has(["SEND_MESSAGES"]))
