...
module.exports = class extends Command {
  ...
  async run(message) {
    ...
    const shipName =
      users[0].username.slice(0, Math.ceil(users[0].username.length / 2)) +
      users[1].username.slice(Math.floor(users[1].username.length / 2));
    ...
    message.channel.sendCustom({
      ...
      embeds: [new MessageEmbed().setDescription(`${users[0].username}`)],
    });
  }
};
