...
module.exports = class extends Command {
  ...
  async run(message) {
    try {
      ...
      if (guild && guild.levelingEnabled === false) {
      ...
      const userRank =
        sortedUsers.findIndex((u) => u.userId === targetUser.id) + 1;
      ...
      ctx.font = "24px Arial";
      ctx.fillText(
        `Level: ${user.level}    Total XP: ${abbreviateNumber(user.xp)}/${abbreviateNumber(requiredXPForNextLevel)}    Rank: ${userRank}`,
        100,
        250,
      );
