    ...
    try {
      ...
      if (!name || !url) {
        return message.reply(
          "Please provide both the emote name and the image URL.",
        );
      }
      ...
      if (fetchedEmotes.some((emote) => emote.name === emoteName)) {
        return message.reply(
          `An emote with the name \`${emoteName}\` already exists in the server.`,
        );
      }

      const emote = await guild.emojis.create(url, emoteName);
      message.channel.sendCustom(
        `Emote ${emote} with name \`${emoteName}\` has been added to the server.`,
      );
    } catch (error) {
      console.error("Error occurred:", error.message);
      message.reply(
        "Failed to add the emote. Please check the image URL and try again.",
      );
    }
  }
};
