  ...
  async run(message, args) {
    ...
    for (const userId of targets) {
      try {
        ...
        if (user.id === message.author.id) {
          results.failed.push(`${user.tag} (yourself)`);
          continue;
        }
    ...
    if (!isBanwave) {
      ...
      const banwaveEmbed = new MessageEmbed()
        ...
        .addFields({
          name: "Users Banned",
          value: results.banned.map((u) => `• ${u}`).join("\n"),
        })
        .setColor(client.color.green)
      ...
      message.channel.sendCustom({ embeds: [banwaveEmbed] }).then(async (s) => {
        if (logging && logging.moderation.delete_reply === "true") {
          setTimeoout(() => s.delete().catch(() => {}), 5000);
        }
      });
    }
    ...
    if (logging && logging.moderation.ban === "true") {
      ...
      if (logChannel) {
        ...
        if (!isBanwave) {
          ...
          send(logChannel, { embeds: [banwaveEmbed] });
        }
