...
module.exports = class GetActivityCommand extends Command {
  ...
  async run(message, args) {
    ...
    try {
      ...
      if (!svgImage) {
        return message.channel.sendCustom("Failed to fetch user's activity.");
      }

      // Sending SVG as a Discord message
      message.channel.sendCustom({ content: `${response}` });
    } catch (error) {
      console.error("Error fetching user activity:", error);
      message.channel.sendCustom("Failed to fetch user's activity.");
    }
  }
};
