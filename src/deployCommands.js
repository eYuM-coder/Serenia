const fs = require("node:fs");
const crypto = require("crypto");
const { REST } = require("@discordjs/rest");
const { Routes } = require("discord-api-types/v10"); // Changed to v10 (Discord.js v14 uses v10)
const { green, yellow, red } = require("colors");

console.clear();

const commands = [];
const commandFolders = fs.readdirSync("./src/slashCommands");

// Read all commands and store them
for (const folder of commandFolders) {
  const commandFiles = fs
    .readdirSync(`./src/slashCommands/${folder}`)
    .filter((file) => file.endsWith(".js"));

  for (const file of commandFiles) {
    try {
      const command = require(`./slashCommands/${folder}/${file}`);
      commands.push(command.data.toJSON());
    } catch (err) {
      console.error(red(`Error loading command ${file}:`, err));
    }
  }
}

// Generate a hash from command data
const commandHash = crypto
  .createHash("sha256")
  .update(JSON.stringify(commands))
  .digest("hex");
const cacheFile = "./commandCache.json";

// Check if cache exists and read the previous hash
let previousHash = null;
if (fs.existsSync(cacheFile)) {
  try {
    const cacheData = JSON.parse(fs.readFileSync(cacheFile, "utf8"));
    previousHash = cacheData.hash;
  } catch (err) {
    console.error(red("Error reading cache:", err));
  }
}

// Only update commands if the hash has changed
if (previousHash === commandHash && commands.length > 0) {
  console.log(yellow("ℹ No changes detected. Skipping command registration."));
} else {
  const rest = new REST({ version: "10" }).setToken(process.env.TOKEN); // Changed to v10

  console.log(green("🔄 Started refreshing application (/) commands."));
  console.log(`📝 Registering ${commands.length} commands...`);

  rest
    .put(Routes.applicationCommands(process.env.MAIN_CLIENT_ID), {
      body: commands,
    })
    .then(() => {
      console.log(
        green(
          `✅ Successfully registered ${commands.length} application commands.`,
        ),
      );
      // Save the new hash
      fs.writeFileSync(
        cacheFile,
        JSON.stringify({ hash: commandHash, timestamp: Date.now() }, null, 2),
      );
    })
    .catch((err) => {
      console.error(red("❌ Failed to register commands:"), err);
    });
}
